...
def process(config):
    ...
    for status_index in range(0, len(subjobids_to_check), 100):
        ...
        for unit in subjobids_to_check[status_index:(status_index + 100)]:
            ...
            current_workunit = workunits[unit['workunit_key']]

            job_key_mapping[f"{current_workunit['status']['job_id']}:{unit['subjob_key']}"] = unit
            job_keys_to_check.append(
                f"{current_workunit['status']['job_id']}:{unit['subjob_key']}")
    ...
    for workunit_key in workunits:
        ...
        for subjob_key in workunit['subjobs']:
            ...
            for collection_key in subjob['collections']:
                total_ligands_in_subjob += int(subjob['collections'][collection_key]['ligand_count'])
    ...
    print("SUMMARY BASED ON AWS BATCH COMPLETION STATUS (different than actual processing status):\n")
