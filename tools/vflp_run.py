...
def perform_isomer_unique_correction(ctx, sterio_smiles_ls):
	...
	limit = int(ctx['config']['rdkit_stereoisomer_max_count'])
	...
	for smi in sterio_smiles_ls:
		...
		try:
			...
			if limit != 0 and len(isomers_canon_unique) == limit:
				break
...
def run_obabel_tautomerization(ctx, stereoisomer):
	...
	if len(stereoisomer['tautomer_smiles']) > int(ctx['config']['obabel_tautomer_max_count']):
